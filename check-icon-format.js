...
console.log(
  "   Type:",
  type === 1 ? "✓ ICO" : type === 2 ? "✓ CURSOR" : "✗ Unknown",
);
...
for (let i = 0; i < count; i++) {
  ...
  console.log(
    `   Image ${i + 1}: ${width}x${height} (${bpp} bpp, ${size} bytes)`,
  );
...
const recommendedSizes = [hasSmall, hasMedium, hasLarge, hasXLarge].filter(
  (x) => x,
).length;
...
if (recommendedSizes < 2) {
  ...
  console.error(
    "   Recommended: Include at least 16x16, 32x32, 48x48, and 256x256.",
  );
  console.error("\n💡 To fix:");
  console.error(
